...
test = pd.read_csv('../../dat/test.tsv', sep='\t')
df_train = train
df_test = test
...
train['category_brand_name'] = train['category_name'] + '/' + train['brand_name']
test['category_brand_name'] = test['category_name'] + '/' + test['brand_name']
le.fit(np.hstack([train.category_brand_name, test.category_brand_name]))
train['category_brand'] = le.transform(train.category_brand_name)
test['category_brand'] = le.transform(test.category_brand_name)
...
raw_text = np.hstack([train.category_name.str.lower(),
                      ...
                      train.category_brand_name.str.lower()])
...
train["seq_category_brand_name"] = tok_raw.texts_to_sequences(train.category_brand_name.str.lower())
test["seq_category_brand_name"] = tok_raw.texts_to_sequences(test.category_brand_name.str.lower())
...
MAX_CATEGORY_BRAND_NAME_SEQ = MAX_CATEGORY_NAME_SEQ + 5  # + Brand = 1 to 5 words
MAX_TEXT = np.max([np.max(train.seq_name.max())
                      , np.max(test.seq_name.max())
                      , np.max(train.seq_category_brand_name.max())
                      , np.max(test.seq_category_brand_name.max())
...
MAX_CATEGORY_BRAND = np.max([train.category_brand.max(), test.category_brand.max()]) + 1
...
def get_keras_data(dataset):
    X = {
            ...
            , 'category_brand': np.array(dataset.category_brand)
            , 'category_brand_name': pad_sequences(dataset.seq_category_brand_name
                                                   , maxlen=MAX_CATEGORY_BRAND_NAME_SEQ)
...
def get_model():
    ...
    category_brand = Input(shape=[1], name="category_brand")

    category_brand_name = Input(shape=[X_train["category_brand_name"].shape[1]],
    ...
    emb_category_brand_name = Embedding(MAX_TEXT, emb_size // 3)(category_brand_name)
    emb_category_brand = Embedding(MAX_CATEGORY_BRAND, 10)(category_brand)
    emb_item_condition = Embedding(MAX_CONDITION, 5)(item_condition)

    rnn_layer1 = GRU(16)(emb_item_desc)
    #rnn_layer2 = GRU(8)(emb_category_name)
    rnn_layer3 = GRU(8)(emb_name)
    rnn_layer4 = GRU(8)(emb_category_brand_name)

    # main layer
    main_l = concatenate([
        Flatten()(emb_category_brand)
    ...
    # model
    model = Model([name, item_desc, category_brand, category_brand_name
