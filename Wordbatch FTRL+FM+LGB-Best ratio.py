...
def aggregate_predicts_N(preds, ratios):
...
valid_combs = []
current_comb = []
def get_valid_combs(possible_ratios):    
    if not possible_ratios:
        if(sum(current_comb) == 1.0):
            valid_combs.append(current_comb.copy())          
            return
    else:
        ratio_range = possible_ratios.pop()
        for r in ratio_range:
            current_comb.append(r*0.01)
            get_valid_combs(possible_ratios.copy())
            current_comb.pop()
                    
                
def get_best_ratios(preds, Y_dev):
    lowest = 0.99
    n = len(preds)
    possible_ratios = []
    for pos in range(n):
        possible_ratios.append(range(101))
    
    
    get_valid_combs(possible_ratios)
    
    for ratios in valid_combs:
        Y_dev_preds = aggregate_predicts_N(preds, ratios)
