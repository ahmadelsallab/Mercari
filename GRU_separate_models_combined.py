...
def get_model_category_name():
    ...
    #model
    model = Model(category_name, output)
    #optimizer = optimizers.RMSprop()
    optimizer = optimizers.Adam()
    model.compile(loss="mse",
                  optimizer=optimizer)
...
def get_model_item_condition():
    ...
    #main layer
    main_l = Flatten() (emb_item_condition)
    main_l = Dropout(0.25)(Dense(512,activation='elu') (main_l))
    main_l = Dropout(0.2)(Dense(64,activation='elu') (main_l))
    ...
    #model
    model = Model(item_condition, output)
    #optimizer = optimizers.RMSprop()
    optimizer = optimizers.Adam()
    model.compile(loss="mse",
                  optimizer=optimizer)
...
K.set_value(model_name.optimizer.lr, lr_init)
K.set_value(model_name.optimizer.decay, lr_decay)

history = model_name.fit(X_train, dtrain.target
                    , epochs=epochs
...
print('[{}] Finished FITTING MODEL NAME...'.format(time.time() - start_time))

model_brand = get_model_brand()
K.set_value(model_brand.optimizer.lr, lr_init)
K.set_value(model_brand.optimizer.decay, lr_decay)

history = model_brand.fit(X_train, dtrain.target
                    , epochs=epochs
                    , batch_size=BATCH_SIZE
                    , validation_split=0.01
                    #, callbacks=[TensorBoard('./logs/'+log_subdir)]
...
print('[{}] Finished FITTING MODEL BRAND...'.format(time.time() - start_time))

model_category = get_model_category()
K.set_value(model_category.optimizer.lr, lr_init)
K.set_value(model_category.optimizer.decay, lr_decay)

history = model_category.fit(X_train, dtrain.target
                    , epochs=epochs
                    , batch_size=BATCH_SIZE
                    , validation_split=0.01
                    #, callbacks=[TensorBoard('./logs/'+log_subdir)]
...
print('[{}] Finished FITTING MODEL CATEGORY...'.format(time.time() - start_time))

model_category_name = get_model_category_name()
K.set_value(model_category_name.optimizer.lr, lr_init)
K.set_value(model_category_name.optimizer.decay, lr_decay)

history = model_category_name.fit(X_train, dtrain.target
                    , epochs=epochs
                    , batch_size=BATCH_SIZE
                    , validation_split=0.01
                    #, callbacks=[TensorBoard('./logs/'+log_subdir)]
...
print('[{}] Finished FITTING MODEL CATEGORY NAME...'.format(time.time() - start_time))

model_item_condition = get_model_item_condition()
K.set_value(model_item_condition.optimizer.lr, lr_init)
K.set_value(model_item_condition.optimizer.decay, lr_decay)

history = model_item_condition.fit(X_train, dtrain.target
                    , epochs=epochs
                    , batch_size=BATCH_SIZE
                    , validation_split=0.01
                    #, callbacks=[TensorBoard('./logs/'+log_subdir)]
...
print('[{}] Finished FITTING MODEL ITEM CONDN...'.format(time.time() - start_time))

model_item_desc = get_model_item_desc()
K.set_value(model_item_desc.optimizer.lr, lr_init)
K.set_value(model_item_desc.optimizer.decay, lr_decay)

history = model_item_desc.fit(X_train, dtrain.target
                    , epochs=epochs
                    , batch_size=BATCH_SIZE
                    , validation_split=0.01
                    #, callbacks=[TensorBoard('./logs/'+log_subdir)]
...
print('[{}] Finished FITTING MODEL ITEM DESC...'.format(time.time() - start_time))
...
def get_model():
    ...
    emb_name = Embedding(MAX_TEXT, emb_size//3, weights=[model_name.layers[1].get_weights()])(name)
    emb_item_desc = Embedding(MAX_TEXT, emb_size, weights=[model_item_desc.layers[1].get_weights()])(item_desc)
    emb_category_name = Embedding(MAX_TEXT, emb_size//3, weights=[model_category_name.layers[1].get_weights()])(category_name)
    emb_brand = Embedding(MAX_BRAND, 10, weights=[model_brand.layers[1].get_weights()])(brand)
    emb_category = Embedding(MAX_CATEGORY, 10, weights=[model_category.layers[1].get_weights()])(category)
    emb_item_condition = Embedding(MAX_CONDITION, 5, weights=[model_item_condition.layers[1].get_weights()])(item_condition)
    ...
    model.compile(loss="mse",
                  optimizer=optimizer)
    model.layers[14].set_weights(model_item_desc.layers[2].get_weights())
    model.layers[15].set_weights(model_category_name.layers[2].get_weights())
    model.layers[16].set_weights(model_name.layers[2].get_weights())
    return model
